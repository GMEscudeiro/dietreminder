...
import { Search, Plus, Save, X, Clock, Type, List, ArrowLeft, Calendar } from 'lucide-react';
...
function DiasSemanaSelector({ diasSelecionados, onChange }) {
  ...
  const todosSelecionados = diasSelecionados.length === DIAS_SEMANA.length;

  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <label className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 ml-1 uppercase tracking-wider flex items-center gap-1.5">
        ...
        {DIAS_SEMANA.map((dia) => {
          ...
          return (
            <button
              ...
              className={`
                flex-1 min-w-[38px] py-2 rounded-xl text-xs font-semibold transition-all duration-150 border
                ${selecionado
                  ? isWeekend
      ...
      <form className="flex flex-col gap-8" onSubmit={(e) => e.preventDefault()}>
        ...
        <section className="flex flex-col gap-5">
          <div className="flex items-center gap-2 pb-2 border-b border-zinc-100 dark:border-zinc-800/60">
            <span className="w-5 h-5 rounded-md bg-zinc-900 dark:bg-zinc-100 flex items-center justify-center">
              <Type size={11} className="text-white dark:text-zinc-900" strokeWidth={2.5} />
            </span>
        ...
        <section className="flex flex-col gap-5">
          <div className="flex items-center gap-2 pb-2 border-b border-zinc-100 dark:border-zinc-800/60">
            <span className="w-5 h-5 rounded-md bg-zinc-900 dark:bg-zinc-100 flex items-center justify-center">
              <Calendar size={11} className="text-white dark:text-zinc-900" strokeWidth={2.5} />
            </span>
            <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 tracking-tight">
              Agendamento
            </h3>
        ...
        <section className="flex flex-col gap-5">
          <div className="flex items-center gap-2 pb-2 border-b border-zinc-100 dark:border-zinc-800/60">
            <span className="w-5 h-5 rounded-md bg-zinc-900 dark:bg-zinc-100 flex items-center justify-center">
              <Search size={11} className="text-white dark:text-zinc-900" strokeWidth={2.5} />
            </span>
            <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 tracking-tight">
              Alimentos
            </h3>
            <span className="ml-auto text-xs font-medium text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800/50 px-2 py-0.5 rounded-full">
        ...
        {/* Ações */}
        <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800/50 flex flex-col-reverse sm:flex-row justify-end gap-3">
          <button
            type="button"
            onClick={onVoltar}
